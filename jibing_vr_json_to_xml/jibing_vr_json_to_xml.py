...
class tools:
	...
	def replace_doctor_pic(self,doctor_name, doctor_url, doctor_pic):
		doctor_name_4_gbk = doctor_name.strip('\t').encode('gbk','ignore')
		if doctor_name_4_gbk in mingyi_url:
			fields = mingyi_url[doctor_name_4_gbk].strip().split("\t")
#尹伟    http://m.baikemy.com/expert/15361207273473/   http://mingyi.sogou.com/fuwu/hospital/79202236/department ...      http://img01.sogoucdn.com...
			if len(fields) < 4:
				return doctor_pic
			if doctor_url == fields[1]:
				if fields[3]:
		...
		output.write('<DOCUMENT>')
 		for line in open(filename): 
			...
			for expert_item in item_dict['expert']:
				...
				itemXml = itemXml.replace('expert_doctor_pic', self.isNone(self.replace_doctor_pic(expert_item['expertName'],expert_item['expertUrl'], expert_item['avatar'])))
...
expert = u'''
	     <expert>
			 ...
			 <expert_pic><![CDATA[expert_doctor_pic]]></expert_pic>
